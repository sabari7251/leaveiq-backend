...
from ai import getIntent, get_openrouter_client, get_weekly_summary
from auth import get_current_user
from config import OPENROUTER_MODEL, UPLOAD_DIR
...
* Leave applications → use leave application tools.
...
def chat(chatreuqest:ChatBot,background_tasks:BackgroundTasks, cursor=Depends(get_db_cursor), current_user=Depends(get_current_user)):
    ...
    with get_openrouter_client() as client:
        ...
        for _ in range(5):
            ...
            # Retry with backoff for rate-limited free-tier models
            llm_response = None
            for attempt in range(3):
                try:
                    llm_response = client.chat.send(
...
def upload_policy(
    ...
    try:
        ...
        for p_file in UPLOAD_DIR.glob("*.pdf"):
            if p_file.name != "temp_policy.pdf":
                try:
                    p_file.unlink()
                except Exception:
                    pass
        ...
        # Clean up temporary resources if they exist
